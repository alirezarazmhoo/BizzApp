					...
					if (this.activityType == 0) {
						...
						if (this.mainPage_RecentActivityUserMediaBusinesses.length == 1) {
							...
							for (var i = 1; i < this.mainPage_RecentActivityUserMediaBusinesses.length; i++) {
								count++;
								img.push(this.mainPage_RecentActivityUserMediaBusinesses[i].image);
							}
...
function ShowModal(elem) {
	$.ajax({
		...
		success: function (response) {
			...
			//alert(response.item.persianDate);
			//alert(response.item.description);
			//alert(response.item.totalReview);
			//alert(response.item.id);
			//$.each(response.item.images, function () {
			//	alert(this);
			//})
			$("#modalbusinessname").text(response.item.name);
